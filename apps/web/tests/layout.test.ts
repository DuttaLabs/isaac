...
import { AIR, N_BK7, OpticalSystem, Surface, type Material } from '@isaac/optical-core';
...
test('sag is the sag of the circle the surface came from', () => {
  for (const radius of [50, -50, 12.5]) {
    for (const y of [0, 1, 5, 10]) {
      assert.ok(
        Math.abs(sag(1 / radius, y) - sagFromCircle(radius, y)) < 1e-12,
        `sag disagrees at R=${radius}, y=${y}`,
      );
    }
  }
  assert.equal(sag(0, 7), 0, 'a plane has no sag');
});
