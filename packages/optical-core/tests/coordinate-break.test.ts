...
function breakSurface(id: string, thickness: number, changes: Partial<CoordinateBreak>): Surface {
  return new Surface({ id, type: 'COORDINATE_BREAK', thickness, coordinateBreak: look(changes) });
}

test('a break with nothing set changes nothing at all', () => {
  const identity = breakSurface('cb', 0, {}).frameChange;
...
test('tilts are right-handed about the positive axes, in degrees', () => {
  ...
  const tilted = breakSurface('cb', 0, { tiltXDeg: 90 }).frameChange;
  assert.ok(tilted.apply(new Point3(0, 1, 0)).equals(new Point3(0, 0, 1), 1e-12));

  // About y, +z onto +x.
  const aboutY = breakSurface('cb', 0, { tiltYDeg: 90 }).frameChange;
  assert.ok(aboutY.apply(new Point3(0, 0, 1)).equals(new Point3(1, 0, 0), 1e-12));

  // About z, +x onto +y.
  const aboutZ = breakSurface('cb', 0, { tiltZDeg: 90 }).frameChange;
...
test('the order flag decides whether the decenter is tilted with the frame', () => {
  ...
  const decenterFirst = breakSurface('a', 0, parameters).frameChange;
  ...
  const tiltFirst = breakSurface('b', 0, { ...parameters, tiltFirst: true }).frameChange;
  assert.ok(tiltFirst.origin.equals(new Point3(0, 0, 10), 1e-12));
});

test('one break undoes another by negating everything and flipping the order', () => {
  // The manual calls this out as the reason the order flag exists, and it is the
  // property that makes a tilted element possible: break, element, un-break.
  const parameters = { decenterX: 3, decenterY: -7, tiltXDeg: 12, tiltYDeg: -5, tiltZDeg: 30 };
  const applied = breakSurface('a', 0, parameters).frameChange;
  const undone = breakSurface('b', 0, {
  ...
  assert.ok(
    ...
    'a break and its inverse leave the frame alone',
  );
});
 ...
 * through a pair of −45° breaks. This is the three-surface idiom the corpus is
 ...
 * mirrors already used, and coordinate breaks needed nothing added to it.
 */
function newtonian(): OpticalSystem {
  return new OpticalSystem({
    surfaces: [
      ...
      breakSurface('cb-in', 0, { tiltXDeg: -45 }),
      new Surface({
        id: 'diagonal',
        type: 'STANDARD',
        thickness: 0,
      ...
      breakSurface('cb-out', 100, { tiltXDeg: -45 }),
...
test('a fold mirror turns the axis, and the surfaces after it follow', () => {
  ...
  // Everything up to the break is still on the axis: the primary sits 800 along
...
test('the axial coordinate is unfolded, so first-order data survives the fold', () => {
  ...
  // breaks contribute no distance, and the bend does not shorten the path.
  ...
  // A break has no power, so the folded telescope has the focal length of its
...
test('a ray follows the fold, and the trace records no interaction at the break', () => {
  const system = newtonian();
  const ray = new Ray(new Point3(0, 10, -10), new Vector3(0, 0, 1), { wavelengthNm: 587.5618 });
  const result = traceRay(system, ray);
  assert.equal(result.status, 'TERMINATED');

  // The breaks are dummies: they meet no ray, so they contribute no
...
test('a coordinate break refuses everything that would make it optical', () => {
  ...
  assert.throws(
    () => new Surface({ id: 'cb', type: 'COORDINATE_BREAK', thickness: 0 }),
    /requires its decenters and tilts/,
  );
  // And the parameters are refused anywhere they would mean nothing.
  assert.throws(
    () => new Surface({ id: 's', type: 'STANDARD', thickness: 0, coordinateBreak: parameters }),
    /only meaningful on a COORDINATE_BREAK/,
  );
});

test('a coordinate break cannot be a boundary between two media', () => {
  assert.throws(
    () =>
      new OpticalSystem({
        surfaces: [
          ...
          new Surface({
            ...
            type: 'COORDINATE_BREAK',
            thickness: 0,
            coordinateBreak: look({ tiltXDeg: 2 }),
            material: AIR,
          }),
